...
// overwriting someone else's concurrent edit. Pass `sha` as null/undefined
// when creating a brand-new file (GitHub creates it instead of updating).
async function writeFile(path, content, sha, message) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`GitHub write failed (${res.status}) for ${path}: ${errBody}`);
  }

  return res.json();
}

// Same as writeFile, but `base64Content` is already base64-encoded (used
// for uploaded images — the browser sends them as base64, so there's no
// UTF-8 text to re-encode).
...
module.exports = { readFile, writeFile, writeBinaryFile };
